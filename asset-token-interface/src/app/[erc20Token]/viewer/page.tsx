...
import SALTViewerClient from "../salt-viewer-client";

export const metadata: Metadata = {
  title: "Investor Portal · Token",
  description: "Portal do investidor ERC-1400",
};
...
export default async function ViewerPage({
  ...
  if (!ERC1400_TOKENS.has(erc20Token)) {
    return (
      <div className="rounded-sm bg-white shadow-md dark:bg-slate-900 min-h-screen flex justify-center items-center">
        <div className="p-6 rounded-lg shadow-lg text-center text-lg font-bold">
          Token does not support the investor portal.
        </div>
      </div>
    );
  }

  return <SALTViewerClient contracts={contracts[erc20Token]} />;
}
