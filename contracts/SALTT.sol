...
/**
 * @title SALTT - Security ALT-T Token (ERC-1400 compatible, ERC-20 retrocompatível)
 ...
 */
contract SALTT is ERC20, ERC20Burnable, Ownable {
    ...
    modifier onlyController() {
        require(controllers[msg.sender] || msg.sender == owner(), "SALTT: not a controller");
        _;
    }

    modifier onlyIssuable() {
        require(isIssuable, "SALTT: issuance is locked");
        _;
    }

    modifier notBlocked(address from, address to) {
        require(!blocked[from], "SALTT: sender is blocked");
        require(!blocked[to], "SALTT: recipient is blocked");
        _;
    }

    // ─── CONSTRUTOR ────────────────────────────────────────────────────────

    constructor() ERC20("503-SALTT", "SALTT") Ownable(msg.sender) {
        ...
        if (!_byPartition) {
            if (from != address(0)) {
                require(
                    ...
                    "SALTT: insufficient unlocked balance"
                );
                _balanceByPartition[from][PARTITION_UNLOCKED] -= value;
            }
            if (to != address(0)) {
    ...
    function redeemByPartition(bytes32 partition, uint256 value, bytes calldata data) external {
        require(_balanceByPartition[msg.sender][partition] >= value, "SALTT: insufficient partition balance");
        _byPartition = true;
        _burn(msg.sender, value);
        _byPartition = false;
    ...
    function controllerTransfer(
        ...
        require(isControllable, "SALTT: controllability revoked");
    ...
    function transferByPartition(
        ...
        require(_balanceByPartition[msg.sender][fromPartition] >= value, "SALTT: insufficient partition balance");
        _byPartition = true;
        _transfer(msg.sender, to, value);
        _byPartition = false;
    ...
    function removeDocument(bytes32 name) external onlyOwner {
        ...
        require(doc.lastModified != 0, "SALTT: document does not exist");
