...
/**
 * @title SALT1 - Security ALT-1 Token (ERC-1400 compatible, ERC-20 retrocompatível)
 ...
 */
contract SALT1 is ERC20, ERC20Burnable, Ownable {
    ...
    modifier onlyController() {
        require(controllers[msg.sender] || msg.sender == owner(), "SALT1: not a controller");
        _;
    }

    modifier onlyIssuable() {
        require(isIssuable, "SALT1: issuance is locked");
        _;
    }

    modifier notBlocked(address from, address to) {
        require(!blocked[from], "SALT1: sender is blocked");
        require(!blocked[to], "SALT1: recipient is blocked");
        _;
    }

    // ─── CONSTRUTOR ────────────────────────────────────────────────────────

    constructor() ERC20("503-SALT-1", "SALT1") Ownable(msg.sender) {
        // Emite o supply total na partição UNLOCKED
        uint256 initialSupply = 1_360_450_000;
        ...
        if (!_byPartition) {
            if (from != address(0)) {
                require(
                    ...
                    "SALT1: insufficient unlocked balance"
                );
                _balanceByPartition[from][PARTITION_UNLOCKED] -= value;
            }
            if (to != address(0)) {
    ...
    function redeemByPartition(bytes32 partition, uint256 value, bytes calldata data) external {
        require(_balanceByPartition[msg.sender][partition] >= value, "SALT1: insufficient partition balance");
        _byPartition = true;
        _burn(msg.sender, value);
        _byPartition = false;
    ...
    function controllerTransfer(
        ...
        require(isControllable, "SALT1: controllability revoked");
    ...
    function transferByPartition(
        ...
        require(_balanceByPartition[msg.sender][fromPartition] >= value, "SALT1: insufficient partition balance");
        _byPartition = true;
        _transfer(msg.sender, to, value);
        _byPartition = false;
    ...
    function removeDocument(bytes32 name) external onlyOwner {
        ...
        require(doc.lastModified != 0, "SALT1: document does not exist");
